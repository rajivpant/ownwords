 ...
 */
function saveImagesSidecar(mdPath, urlToLocal, results) {
  ...
  const sidecar = {
    _meta: {
      ...
      stats: {
        total: Object.keys(urlToLocal).length,
        downloaded: results.downloaded,
        failed: results.failed,
        totalSize: results.totalSize
      }
    },
    images: Object.entries(urlToLocal).map(([url, filename]) => ({
      originalUrl: url,
      localFile: filename
    })),
...
async function fetchViaApi(urlOrSlug, outputDir, options = {}) {
  ...
  let frontMatter = generateEnrichedFrontMatter(normalized);
  ...
  if (images) {
    ...
    if (allImageUrls.length > 0) {
      ...
      if (Object.keys(urlToLocal).length > 0) {
        ...
        // Save images sidecar
        imagesSidecarPath = saveImagesSidecar(mdPath, urlToLocal, results);
        if (!silent) {
