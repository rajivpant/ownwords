...
// NOTE: Self-linking images ([![alt](img)](img)) are preserved from WordPress
// This is intentional - WordPress uses this pattern for lightbox/gallery functionality.
// The image-utils.js rewriteImageUrls function handles rewriting both src and href
// attributes so self-linking images work correctly after publishing back to WordPress.

// Custom rule to preserve YouTube iframes
// Turndown in Node.js doesn't have full DOM like querySelector, so we match iframes directly
turndownService.addRule('youtubeIframe', {
  ...
  replacement: function (content, node) {
    const src = node.getAttribute('src') || '';
    const title = node.getAttribute('title') || 'YouTube video';
    ...
    // Output as a YouTube link with thumbnail that renders nicely
    const youtubeUrl = `https://www.youtube.com/watch?v=${videoId}`;

    // Return markdown with linked thumbnail image for visual preview
    return `\n\n[![${title}](https://img.youtube.com/vi/${videoId}/maxresdefault.jpg)](${youtubeUrl})\n\n`;
  }
});
...
function extractMetadata(html) {
  ...
  for (const pattern of datePatterns) {
    ...
    if (match) {
      // Preserve full timestamp if available (e.g., 2025-12-07T15:30:00)
      // Only truncate if it's just a date with extra text (e.g., from generic YYYY-MM-DD match)
      const dateStr = match[1];
      if (dateStr.length > 10 && dateStr[10] === 'T') {
        // Full ISO timestamp - preserve it
        date = dateStr;
      } else {
        // Simple date or date with other text - extract just the date
