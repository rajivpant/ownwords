 ...
 *
 * Replaces local image paths in both src and href attributes.
 * href rewriting is needed for clickable images ([![alt](img)](img))
 * where the link wraps the figure element.
 *
...
function rewriteImageUrls(html, urlMap) {
  ...
  for (const [localPath, wpUrl] of Object.entries(urlMap)) {
    ...
    result = result.replace(new RegExp(`src='${escaped}'`, 'g'), `src='${wpUrl}'`);
    // Replace in href attributes for clickable images ([![alt](img)](img) pattern)
    result = result.replace(new RegExp(`href="${escaped}"`, 'g'), `href="${wpUrl}"`);
    result = result.replace(new RegExp(`href='${escaped}'`, 'g'), `href='${wpUrl}'`);
  }
...
/**
 * Read content type from front matter
 * Used to validate that --type flag matches the frontmatter type field
 *
 * @param {string} mdPath - Path to markdown file
 * @returns {string|null} Content type ('page' or null if not specified)
 */
function readFrontMatterType(mdPath) {
  if (!fs.existsSync(mdPath)) {
    return null;
  }
